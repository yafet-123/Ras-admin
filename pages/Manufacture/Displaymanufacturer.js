...
export async function getServerSideProps(context) {
	...
	const data = await authaxios.get(`${api}/manufacturer/`)
  	return {
    	props: {
...
export default function Displaymeasurement({manufacturer}) {
    ...
    const [Website, setWebsite] = useState("")
  	...
  	const handleupdate = async (e)=>{
      	e.preventDefault()
      	await authaxios.patch(`${api}/manufacturer/${id}`,{
      		Name:Name,
						        			...
						        			<tr key={index} className="bg-white p-3">
						        				...
						        				<td>
						        					<Button variant="warning" onClick={(index)=>{
														...
														setisin(data.isin)
														setHeadQuarterLocation(data.HeadQuarterLocation)
